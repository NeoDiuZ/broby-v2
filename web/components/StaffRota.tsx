...
type Schedule = {rooms: string[]; availability: Record<string, Record<string, Shift[]>>; date_overrides: Record<string, Record<string, Exception>>};
...
function RotaEditor({config, onClose}: {config?: Row; onClose: () => void}) {
 ...
 const [draft, setDraft] = useState<Schedule>(() => structuredClone({rooms: config?.data.rooms || [], availability: config?.data.availability || {}, date_overrides: config?.data.date_overrides || {}}));
 ...
 return <Modal title="Edit staff rota" onClose={() => {if (!busy) onClose()}} wide>
  <div className="modal-body rota-editor">
   <p>Times use {w.snapshot?.clinic.data.timezone || 'Asia/Singapore'}. Gaps between shifts are breaks. Dated changes replace that day’s weekly hours. Existing appointments must fit before changes can be saved.</p>
   <fieldset disabled={busy} className="rota-controls">
    ...
    <h3>Leave and replacement hours</h3>
...
export function StaffRota() {
 ...
 const [open, setOpen] = useState(false), [editing, setEditing] = useState(false), [start, setStart] = useState('');
