...
export type Snapshot={clinical_verification?:{epoch:string;verified_at:string;restricted_patients:string[]};records:Row[];actor:Row;clinic:Row;clinics:(Row&{member_id?:string})[];jobs:any[];permissions:string[];read_permissions?:string[];integrations:Record<string,boolean>;mode:string;snapshot_revision?:string};
