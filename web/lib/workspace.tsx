...
type Context={snapshot:Snapshot|null;records:Row[];error:string;loading:boolean;offline:boolean;pending:any[];refresh:()=>Promise<void>;act:(a:string,p:any,key?:string)=>Promise<any>;switchIdentity:(clinic:string,actor:string)=>void;notify:(text:string)=>void;toast:string};
...
export function Workspace({children}:{children:React.ReactNode}){
 ...
 const act=useCallback(async(a:string,p:any,key?:string)=>{try{const result=await command(a,p,key);await refresh();return result}catch(e){notify((e as Error).message);throw e}},[refresh,notify]);
