...
const listeners=new Set<()=>void>();let recorder:MediaRecorder|null=null;let stream:MediaStream|null=null;let timer:ReturnType<typeof setInterval>|null=null;let pendingWrites:Promise<void>[]=[];let activeMeta:any=null;
...
export async function startRecording(patientId:string,consultationId:string){
 ...
 recorder=new MediaRecorder(stream,mime?{mimeType:mime}:undefined);activeMeta.mime=recorder.mimeType;await localPut('recordings',activeMeta);pendingWrites=[];
 recorder.ondataavailable=e=>{if(!e.data.size)return;const index=state.chunks;emit({chunks:index+1});pendingWrites.push(localPut('audio',{id:id+':'+index,recordingId:id,index,blob:e.data}));};
...
export async function stopRecording(){if(!recorder||!['recording','paused'].includes(state.status))return;emit({status:'saving'});if(timer)clearInterval(timer);await new Promise<void>(resolve=>{recorder!.onstop=()=>resolve();recorder!.stop()});stream?.getTracks().forEach(t=>t.stop());await Promise.all(pendingWrites);activeMeta={...activeMeta,status:'queued',elapsed:state.elapsed,expected:state.chunks};await localPut('recordings',activeMeta);emit({status:'saved'});try{await syncAudio()}catch(e){emit({error:'Voice note is saved on this device. Upload will retry when connected.'})}}
let syncing=false;
export async function syncAudio(){if(syncing)return;syncing=true;try{const i=identity();for(const meta of await localAll('recordings')){if(meta.clinic!==i.clinic||meta.actor!==i.actor||['recording','importing'].includes(meta.status))continue;let serverId=meta.serverId;
...
export async function importAudio(file:File,patientId:string,consultationId:string){
 ...
 for(let index=0;index<expected;index++)await localPut('audio',{id:id+':'+index,recordingId:id,index,blob:file.slice(index*size,(index+1)*size)});
 await localPut('recordings',{...meta,status:'queued'});await syncAudio();
}
