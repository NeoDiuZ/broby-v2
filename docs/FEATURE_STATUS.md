...
| 8 | One action layer for UI and AI | **Implemented.** Shared authorized, versioned, idempotent audited mutations; AI proposals require confirmation. Twelve routine actions have strict proposal schemas, reference checks and labelled reviews, including eight newly advertised operations. The catalogue covers 61 of 91 shared actions; the remaining contracts and broader intent evaluation are unfinished. See ASSISTANT_OPERATIONS.md. |
...
| 27 | Scheduling | **Partial.** Day/week/month, recurring/reschedule/status and room collision checks. A complete weekday editor, split shifts/breaks, dated leave/replacement hours and staff rota view are implemented, with conflict previews and atomic protection for existing bookings. Approved cancellation/no-show/deposit policies, leave approval and rota effective-date ranges remain. See STAFF_ROTA.md and RELEASE_WORK.md for acceptance. |
| 28 | Billing | **Partial.** Integer-cent invoices, explicit discount/tax, PDFs, external payment/refund records and voids. Credit-note issue/reversal, explicit net/tax credits, refund-due balances, printable notes and a CSV credit register are implemented and hosted-verified on synthetic records. Full accounting export/reconciliation and clinic-approved tax/accounting rules remain. See CREDIT_NOTES.md. |
...
| 34 | Reporting | **Partial.** Clinical/financial counts, net payments, CSV export and handover. Credit notes and reversals now contribute to period charge totals; a scoped credit register is available (hosted-verified on synthetic records). Full operational reports, audited accounting reports and clinic-scale performance are unfinished. |
...
| 43 | Share / grant to a new clinic | **Partial.** Reviewed clinic transfers copy approved events/files, opt-in original audio and externally recorded medication history. Stable origin mapping skips repeats; changed data appends reviewed revisions without changing local edits or stock. Hosted two-clinic acceptance passed. Pre-revision imports can establish a reviewed current-source baseline with explicit duplicate-risk acknowledgement; isolated browser/PostgreSQL acceptance passed. Historical equivalence cannot be reconstructed. Verified owner identity and reconciliation of independently created destination patients remain. See CLINIC_TRANSFERS.md. |
...
**Engineering that can continue without a new vendor:** offline cold start/device privacy; long-session physical-device acceptance; verified owner account flows (delivery channel needed for final verification); reconciliation of independently created clinic patients; PMS transition to one canonical PostgreSQL store; independent workers/object storage; pagination/load testing; operational diagnostics/alerts; fuller assistant/action contracts, reporting and test coverage. These are unfinished work, not things an AI agent is inherently unable to build.
