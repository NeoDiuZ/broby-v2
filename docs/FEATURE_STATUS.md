# Broby V2 requirements audit — 26 September 2026
...
Latest application acceptance: [PR #85](https://github.com/NeoDiuZ/broby-v2/pull/85)
is deployed as `71ba6d996b70f15144810044981189b7b5668303` on both V2 services.
It adds exact source-backed numeric assistant/saved charts, literal/exact record
reads and reviewed clinical staff notifications. Full local suites passed
1,332 backend tests plus one expected skip in each database mode, 62 frontend
tests, type checking, production build and all six CI jobs. Hosted synthetic
acceptance passed 24 intent cases, four exact read/chart cases, all 14 existing
feature groups and 17 persistence checks. External clinical notifications remain
disabled on hosted V2; primary/backup/fallback and staff acknowledgement were
verified only in a disposable local browser/HTTP drill. None is real-clinic sign-off.
...
| 19 | Visual analytics | **Implemented for defined views.** Numeric trends with supplied reference bands, source navigation and saved count/group queries. PR #85 adds assistant-to-Reports numeric chart save/refresh/reload parity with exact code/unit/date filters, complete point receipts, preserved zero and explicit missing ranges; hosted synthetic browser and real-model checks passed. Broader operational analytics and performance at clinic scale remain. |
...
| 32 | Staff | **Implemented for the listed account flows.** Nurse/vet/admin access, activation/revocation, manual single-use invitations, password change, TOTP and recovery codes. Account flows passed isolated tests and ten hosted synthetic checks, including invitation replay, MFA/recovery reuse, password/session revocation, migration replay/conflict and AI proposal review. Real staff onboarding/recovery acceptance remains; email delivery is additional work if selected as the required channel. |
...
| 38 | Pet profiles | **Implemented for shared access.** Approved shared profile and saved multi-pet browser access. Owner/household verification and recovery require approved policy and acceptance. Separate cross-device account management is additional product work only if required. |
| 39 | Record vault | **Implemented for shared access.** Approved notes/files/labs; unapproved clinical facts hidden. Owner-authority acceptance and actual migrated media remain; a separate cross-device account is not required by the first-tap source wording. |
...
| 48 | Emergency escalation | **Partial.** Urgent questions and expired clinic-defined acknowledgement targets create one internal alert, with review/closure history and interrupted-worker recovery. PR #85 adds a separate reviewed primary/backup policy and durable notification dispatcher with receipt, acknowledgement, timeout, supersession and fallback handling. Root verified the local browser/real loopback HTTP drill through named backup acknowledgement. Nine hosted checks verify transport and policy remain disabled. Approved HTTPS receivers, private routes, separate supervision and actual named staff response acceptance remain required. No emergency monitoring is claimed. |
...
**Remaining engineering and acceptance:** physical-device crash/eviction and long-session tests; owner/household verification and recovery acceptance, with a separate account flow only if the approved policy requires one; unresolved clinical-source lineage and a proven hold-release contract; shared storage and independently supervised hosted workers; authoritative dependency/integrity indexing before bounded workspace pagination; hosted load/soak testing; approved external alert receivers and an actual staffed incident drill; broader factual assistant composition and representative workflow/language evaluation. PRs #83–85 implemented operational alert transport, immutable clinical dispositions/holds, exact chart reads and reviewed clinical primary/backup notifications; their local/hosted verification does not close the listed operational or human acceptance gates. Inputs and per-item status are recorded in VISION_ACCEPTANCE_2026-09-26.md.
