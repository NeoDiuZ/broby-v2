...
Continuous recording refinement is implemented on `codex/continuous-speech`. Eleven added backend scenarios and fourteen total frontend recording fault tests pass; the full backend suite has 161 passing tests. See CONTINUOUS_SPEECH.md. Hosted provider/browser verification and merge/deployment are the next release gates.
