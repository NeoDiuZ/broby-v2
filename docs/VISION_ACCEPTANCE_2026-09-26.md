...
Both Railway V2 services were verified at code release `84a32d9befbe3eeb6aee74169440839cc5c1a457` (PR #84). Later acceptance changes and their final deployment are recorded in the release evidence below. The detailed implementation history remains in
...
| 8 | Shared UI/AI actions | Partial | Shared authorized executor; 74 strict direct actions, 28 guided operations and six internal adapters; exact reviewed recall/admin actions | Verify guided review coverage and broader intent evaluation | Representative staff workflows and reviewer |
...
| 19 | Visual analytics | Complete | Numeric trends with supplied bands and source navigation; saved grouped counts | Agreed-workload and real-clinic usability acceptance | Clinical reviewer and representative workload |
...
| 23 | Patient AI chat | Partial | Saved private chat, exact owner invoice/reminder and medication reads; preserved duplicate-patient question; real-model synthetic corpus | Broader factual composition, guided workflow acceptance and clinical-language evaluation | Representative questions/samples and clinician |
...
| 25 | AI-driven dashboard | Complete | Confirmed saved queries share deterministic executor; filter/count/browser reload parity | Broader query composition follows #23/#24 | Report/workflow reviewer |
...
| 48 | Emergency escalation | Partial | Durable internal alerts, acknowledgements, target expiry and recovery | External clinical alert, primary/backup, timeout and fallback acceptance | Named on-call rota, channel, response targets, fallback and test participants |
